...
class IntArray():
    ...
    def insert(self, index:int, val:int) -> None:
        ...
        self._size += 1
        new_resmem = ReservedMemory(self._size*self._bytes_per_element)
        insert_pos = index * self._bytes_per_element
        new_resmem.copy(self._resmem, count=insert_pos, source_index=0, destination_index=0)
        right_count = (self._size-1)*self._bytes_per_element-insert_pos
        new_resmem.copy(
            self._resmem,
            count=right_count,
            source_index=insert_pos,
            destination_index=insert_pos + self._bytes_per_element)
        self._resmem = new_resmem
