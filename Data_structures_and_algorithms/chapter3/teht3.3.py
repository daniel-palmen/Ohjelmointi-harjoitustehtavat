...
class SinglyLinkedList():
    ...
    def remove(self, index):
        if index < 0 or index > self._size:
            raise(ValueError('Index out of bounds'))
        previous_node = None
        removable_node = self._head
        for _ in range(index):
            previous_node = removable_node
            removable_node = removable_node.next
        # If remove from front, update head
        if previous_node is None:
            self._head = removable_node.next
        else:
            # If not, update previous node
            previous_node.next = removable_node.next
        # If remove at the end, update tail
        if removable_node == self._tail:
            self._tail = previous_node

        val = removable_node.data
        del(removable_node)
        self._size -= 1
        return val
